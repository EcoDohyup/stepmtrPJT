<template>
  ...
  export default {
    ...
    methods: {
      ...
      async sendSpeed() {
        try {
          const response = await fetch(`http://192.168.0.98/motor/speed?value=${this.speed}`, {
            method: 'GET',
            mode: 'cors',
            headers: {
      ...
      async fetchDistance() {
        try {
          const response = await fetch('http://192.168.0.98/motor/distance', {
            method: 'GET',
            mode: 'cors',
            headers: {
              'Content-Type': 'text/plain'
          ...
          this.distance = await response.text(); // Update distance
        } catch (error) {
    ...
    mounted() {
      ...
      setInterval(this.fetchDistance, 1000);
    },
    beforeUnmount() {
